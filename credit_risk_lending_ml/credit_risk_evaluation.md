# Data Pipeline, Model Evaluation & Governance Report

**Module Target:** Credit Scoring & Behavioral Anomaly Detection  
**Evaluation Mode:** Deterministic / Reproducible Evaluation Run (`random_state=42`)

---

## 1. Data Generation & Verification

Running `generate_data.py` reproducibly outputs the primary datasets with the following parameters:

* **`credit_applicants.csv`:**
  * **Total Records:** 400 applicant profiles
  * **Measured Default Rate:** 20.25% (within target range of 15%–25%)
  * **Missing Values:** Exactly 80 records (20.00%) missing `credit_bureau_score` (Thin-file applicants)
* **`txn_behaviour.csv`:**
  * **Total Records:** 265 transactions
  * **Seeded Behavioral Anomalies:** Exactly 15 transactions prefixed with `BTXNA*` ($\sim 5.66\%$ contamination rate)
...
2. **Stratified Train/Test Split:** Applied a 75/25 stratified split on the target variable (`default`) using `random_state=42` to guarantee identical class distributions across splits:
   * **Train set:** 300 applicants (61 defaults, 239 non-defaults)
   * **Test set:** 100 applicants (20 defaults, 80 non-defaults)
...
Using predicted default probabilities ($P_{	ext{default}}$) from Logistic Regression, test applicants were binned into 4 equal-sized risk tiers (quartiles). The observed default rates demonstrate clear, monotonic risk escalation across tiers:

| Risk Tier | Applicant Count | Probability Range ($P_{	ext{default}}$) | Observed Defaults | Observed Default Rate | Illustrative APR |
| :--- | :---: | :---: | :---: | :---: | :---: |
...
`IsolationForest` was fitted on standardized behavioral features (`txn_hour`, `is_new_device`, `txn_amount_inr`) with a contamination rate matched to the ground truth ($rac{15}{265} pprox 0.0566$):
...
## 6. Bias-Awareness & Governance Framework
...
### Justification:
* **ROC-AUC Superiority:** Logistic Regression achieved an AUC of **0.7188**, significantly outperforming the Decision Tree (**0.5188**), which suffered from severe overfitting on training data.
* **Monotonic Risk Grading:** Logistic Regression produced calibrated probabilities that separated risk into monotonic tiers (8% default in Tier 1 vs. 40% in Tier 4), enabling effective risk-based pricing.
* **Interpretability:** Provides transparent, auditable feature coefficients required for fair lending compliance and regulatory adverse action notices.
